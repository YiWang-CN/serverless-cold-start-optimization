...
# plt.title(key[:5] + f' Container arrival time interval sequence (order less than {limit_xmax})', fontsize=18)
# plt.xlim(xmin=0, xmax=min(limit_xmax, len(selected_diff_sequence)))  # 限制x轴范围
...
# fig.savefig(figure_url + f"diff_sequence_order_less_{limit_xmax}.png")
# plt.close()

# # 到达间隔的时序图, 使用 diff_interval_threshold，查看大于阈值的细节
# diff_interval_threshold = 25000   ###
# # dotted_line_y = 29920    ###
# diff_sequence_mean = np.mean(diff_sequence)
# diff_sequence_variance = np.var(diff_sequence)
# fig = plt.figure(figsize=(15, 10))
...
# # 到达时刻的时序图，查看部分
# fig = plt.figure(figsize=(15, 10))
# sequence_part = sequence[:100]   ###
# plt.plot(range(len(sequence_part)), sequence_part)
# plt.xlabel('Arrival order', fontsize=18)  # 调整x轴标签字体大小为18
...
# 到达时间二次差分的时序图，不限制，查看整体
dotted_line_y = 2000   ###
diff_interval_threshold = float('inf')
diff_sequence_mean = np.mean(second_diff_sequence)
diff_sequence_variance = np.var(second_diff_sequence)
fig = plt.figure(figsize=(15, 10))
# 布尔索引，选择y轴小于diff_interval_threshold的部分
selected_diff_sequence = [diff for diff in second_diff_sequence if diff < diff_interval_threshold]
selected_indices = [i for i, diff in enumerate(second_diff_sequence) if diff < diff_interval_threshold]
plt.plot(selected_indices, selected_diff_sequence)
# plt.axhline(y = dotted_line_y, color='r', linestyle='--')
# plt.text(0, dotted_line_y, f'{dotted_line_y}', fontsize=15, color='black', ha='right', va='center')  # 添加对应的y轴坐标

plt.xlabel('order', fontsize=18)
plt.ylabel('arrival time second diff(ms)',fontsize=18)
plt.title(key[:5] + f' Container arrival time second diff sequence (less than {diff_interval_threshold} ms)', fontsize=18)
plt.xticks(fontsize=16)
plt.yticks(fontsize=16)
ymin, ymax = plt.gca().get_ylim()
xmin, xmax = plt.gca().get_xlim()
...
# 格式化文本并添加到 plt.text() 中
plt.text(xmax*0.75, ymax*0.95, f'{mean_label:<{max_label_length}} {diff_sequence_mean:<.2f}\n{variance_label:<{max_label_length}} {diff_sequence_variance:<.2f}', fontsize=15, color='black', ha='left', va='top')  # 添加对应的坐标
fig.savefig(figure_url + f"2diff_sequence_less_{diff_interval_threshold}_ms.png")
plt.close()
