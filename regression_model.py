...
class Regression(nn.Module):
    def __init__(self, input_size=1, train_window=50, output_size=1, batch_size=1):
        super().__init__()
        self.batch_size = batch_size
        self.train_window = train_window
        self.input_size = input_size
        self.linear1 = nn.Linear(input_size, 128)
        self.linear2 = nn.Linear(128 * train_window, output_size)

    def forward(self, input_seq):        
        # 对序列中的每个时间步应用线性层
        linear1_out = self.linear1(input_seq)  # (batch_size, train_window, 128)
        # 将每个时间步的输出展平
        linear1_out = linear1_out.view( input_seq.size(0), -1)  # (batch_size, train_window*128)
        # 应用第二个线性层
        predictions = self.linear2(linear1_out)  # (batch_size, output_size)
        return predictions
...
if __name__ == "__main__":
    ...
    epoch_vision = 'v10'
    discard = 0
    model_url = os.path.dirname(os.path.realpath(__file__)) + '/regression_model/' + dataset_name +f'/tw={train_window}_'+ key +'/'
...
##########################################################
    # # 绘制输入图形
    # input_figure_url = os.path.dirname(os.path.realpath(__file__)) + '/regression_input_figure/' + f'{dataset_name}/'+ key + '/'
...
##########################################################
    ...
    # 创建regression类的对象，定义损失函数和优化器
    model = Regression(batch_size=batch_size)
    optimizer = torch.optim.Adam(model.parameters(),lr = lr)
    loss_function = nn.MSELoss().to(device)
    print("regression模型初始化完成，开始训练")
    # 统计模型参数数量
...
#         csv_url = os.path.dirname(os.path.realpath(__file__)) + '/csv_directory/'+ f'{dataset_name}/'+ key + '/' + 'regression/' + f'tw={train_window}_{epoch_vision}' + '/'
...
#                 writer.writerow([i, seq, out])
    ...
    #     output_figure_url = os.path.dirname(os.path.realpath(__file__)) + '/regression_output_figure/' + f'{dataset_name}/'+ key + '/' + epoch_vision + '/'
...
##########################################################################
    ...
    # predict_sequence_figure_url = os.path.dirname(os.path.realpath(__file__))+'/predict_sequence_figure/' +dataset_name+'/' +key+'/' +f'tw={train_window}_' + epoch_vision +'/' 
