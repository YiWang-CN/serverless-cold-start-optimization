...
def plot_loss(train_loss, val_loss, save_path):
    ...
    plt.plot(train_loss, label='Training batch Loss')
...
def training(epochs, train_inout_seq, val_inout_seq, model, optimizer,batch_size,model_url):
    ...
    for i in range(1,epochs+1):
        ...
        for batch_idx in range(0, int(len(train_inout_seq)/batch_size)):
            ...
            # 每10批或最后一批次 保存模型参数和画三个损失函数：1~n，100~n，倒数50轮
            if (batch_idx+1)%10 == 0 or batch_idx == int(len(train_inout_seq)/batch_size):
                ...
                if len(loss_list) > 500:
                    plot_part_loss(loss_list, val_loss, save_path + 'last_500_loss.png',start=len(loss_list)-500,end=len(loss_list)) #绘制倒数200轮损失函数
...
if __name__ == "__main__":
    ...
    lr = 0.0001
    epoch_vision = 'v16'
    batch_vision = 'batch490'
    discard = 0
    model_url = os.path.dirname(os.path.realpath(__file__)) + '/lstm_models/' + dataset_name +f'/batchs_diff_cleaned_128_tw={train_window}_'+ key +'/'
...
#画图
    ...
    # for i in range(0, 500, num_plots_per_figure):
    #     start_index = i
